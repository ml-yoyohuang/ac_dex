...
export default {
  ...
  computed: {
    ...
    ...Vuex.mapGetters([]),
    imagesPath() {
      return this.cds.map((item) => require(`img/musics/${item.id}.png`));
    },
    io() {
      const obj = {
    ...
    checkedCds() {
      const found = this.cds.filter((element) => this.checked.indexOf(element.id) > -1);
...
.fossil.page-container
  ...
  template(v-else)
    ...
    .row.gutter-10(:class="`filter${filter}`")
      .col.col-6.col-sm-4.col-md-3.col-lg-2.align-items-stretch(v-for="(item, index) in cds" :class="{'is-checked' : isChecked(item.id)}")
    ...
    .p 複製文字：有的
    .p.textarea
      template(v-for="(item, index) in checkedCds") {{item.name}}
        br
    .p 複製文字：沒有的
    .p.textarea
      template(v-for="(item, index) in notCheckedCds") {{item.name}}
        br
</template>
